...
login = r.login('<YOUR EMAIL>', 'Harrison123!')
...
def get_watchlist_symbols():
    ...
    my_list_names = []
    symbols = []
    for name in ["My First List"]: #r.get_all_watchlists(info='name'):
        my_list_names.append(name)
    for name in my_list_names:
        theList = r.get_watchlist_by_name(name)['results']
        for item in theList:
            print(item)
            #instrument_data = r.get_instrument_by_url("https://api.robinhood.com/instruments/" + item.get('id'))
...
def get_portfolio_symbols():
    ...
    for item in holdings_data:
        if not item:
            continue
        instrument_data = r.get_instrument_by_url(r.get_instruments_by_symbols(item)[0]['url'])
        symbol = instrument_data['symbol']
...
def five_year_check(stockTicker):
    ...
    if ((pd.Timestamp("now") - pd.to_datetime(list_date)) < pd.Timedelta("5 Y")):
        return True
    fiveyear = r.get_stock_historicals(stockTicker,span='5year',bounds='regular')
    closingPrices = []
...
def golden_cross(stockTicker, n1, n2, days, direction=""):
    ...
    #    return False
    history = r.get_stock_historicals(stockTicker,span='month',bounds='regular')
    #history
    closingPrices = []
    dates = []
    for item in history:
